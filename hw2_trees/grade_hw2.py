#!/usr/bin/env python3
"""
Grading script for HW2: Trees & Ensembles.

Usage:
    python grade_hw2.py path/to/student_hw2_trees.ipynb
...
trees_tests.py. Any test that doesn't run (e.g. because the student
...
# Import the canonical registry (this script lives next to trees_tests.py)
sys.path.insert(0, str(Path(__file__).parent.resolve()))
from trees_tests import TEST_REGISTRY, TOTAL_EXPECTED_TESTS
...
def run_notebook_and_grade(notebook_path: Path, work_dir: Path) -> dict | None:
    ...
    # Inject a grading cell that uses TEST_REGISTRY from our trees_tests
    grading_cell_source = '''
import json as _json
import matplotlib as _matplotlib
_matplotlib.use("Agg")

from trees_tests import TEST_REGISTRY
...
def main():
    parser = argparse.ArgumentParser(
        description="Grade a student's HW2 trees & ensembles notebook."
    )
    parser.add_argument(
        "notebook",
        type=str,
        help="Path to the student's hw2_trees.ipynb",
    )
    parser.add_argument(
        "--verbose",
        "-v",
    ...
    hw2_dir = Path(__file__).parent.resolve()

    # Create a temporary working directory with our infrastructure + student notebook
    with tempfile.TemporaryDirectory(prefix="hw2_grade_") as tmpdir:
        work_dir = Path(tmpdir)

        # Copy our infrastructure
        shutil.copy2(hw2_dir / "trees_helpers.py", work_dir / "trees_helpers.py")
        shutil.copy2(hw2_dir / "trees_tests.py", work_dir / "trees_tests.py")
        shutil.copytree(hw2_dir / "test_data", work_dir / "test_data")

        # Copy student notebook
        target_notebook = work_dir / "hw2_trees.ipynb"
