#!/usr/bin/env python3
"""
Grading script for HW4: Dimensionality Reduction.

Usage:
    python grade_hw4.py path/to/student_hw4_dimred.ipynb
...
dimred_tests.py. Any test that doesn't run (e.g. because the student
...
# Import the canonical registry (this script lives next to dimred_tests.py)
sys.path.insert(0, str(Path(__file__).parent.resolve()))
from dimred_tests import TEST_REGISTRY, TOTAL_EXPECTED_TESTS
...
def run_notebook_and_grade(notebook_path: Path, work_dir: Path) -> dict | None:
    ...
    # Inject a grading cell that uses TEST_REGISTRY from our dimred_tests
    grading_cell_source = '''
import json as _json
import matplotlib as _matplotlib
_matplotlib.use("Agg")

from dimred_tests import TEST_REGISTRY
...
def main():
    parser = argparse.ArgumentParser(
        description="Grade a student's HW4 dimensionality reduction notebook."
    )
    parser.add_argument(
        "notebook",
        type=str,
        help="Path to the student's hw4_dimred.ipynb",
    )
    parser.add_argument(
        "--verbose",
        "-v",
    ...
    hw4_dir = Path(__file__).parent.resolve()

    # Create a temporary working directory with our infrastructure + student notebook
    with tempfile.TemporaryDirectory(prefix="hw4_grade_") as tmpdir:
        work_dir = Path(tmpdir)

        # Copy our infrastructure
        shutil.copy2(hw4_dir / "dimred_helpers.py", work_dir / "dimred_helpers.py")
        shutil.copy2(hw4_dir / "dimred_tests.py", work_dir / "dimred_tests.py")
        shutil.copytree(hw4_dir / "test_data", work_dir / "test_data")

        # Copy student notebook
        target_notebook = work_dir / "hw4_dimred.ipynb"
