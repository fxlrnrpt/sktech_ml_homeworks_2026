#!/usr/bin/env python3
"""
Grading script for HW1: Regression & Classification.

Usage:
    python grade_hw1.py path/to/student_hw1_regression.ipynb
...
regression_tests.py. Any test that doesn't run (e.g. because the student
...
# Import the canonical registry (this script lives next to regression_tests.py)
sys.path.insert(0, str(Path(__file__).parent.resolve()))
from regression_tests import TEST_REGISTRY, TOTAL_EXPECTED_TESTS
...
def run_notebook_and_grade(notebook_path: Path, work_dir: Path) -> dict | None:
    ...
    # Inject a grading cell that uses TEST_REGISTRY from our regression_tests
    grading_cell_source = '''
import json as _json
import matplotlib as _matplotlib
_matplotlib.use("Agg")

from regression_tests import TEST_REGISTRY
...
def main():
    parser = argparse.ArgumentParser(
        description="Grade a student's HW1 regression notebook."
    )
    parser.add_argument(
        "notebook",
        type=str,
        help="Path to the student's hw1_regression.ipynb",
    )
    parser.add_argument(
        "--verbose",
        "-v",
    ...
    hw1_dir = Path(__file__).parent.resolve()

    # Create a temporary working directory with our infrastructure + student notebook
    with tempfile.TemporaryDirectory(prefix="hw1_grade_") as tmpdir:
        work_dir = Path(tmpdir)

        # Copy our infrastructure
        shutil.copy2(hw1_dir / "regression_helpers.py", work_dir / "regression_helpers.py")
        shutil.copy2(hw1_dir / "regression_tests.py", work_dir / "regression_tests.py")
        shutil.copytree(hw1_dir / "test_data", work_dir / "test_data")

        # Copy student notebook
        target_notebook = work_dir / "hw1_regression.ipynb"
